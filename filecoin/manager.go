...
func jsonReturn(code cid.Cid, method abi.MethodNum, ret []byte) (interface{}, error) {
...
func (wm *WalletManager) GetTransactionReceiptData(txCid string) (map[string]interface{}, error) {
	callMsg := map[string]interface{}{
		"/": txCid,
	}
	...
	result, err := wm.WalletClient.Call("Filecoin.StateSearchMsg", params)
	if err != nil {
		return nil, err
	}
	if result.Get("Receipt").Value() != nil {
		result, err = wm.WalletClient.Call("Filecoin.StateWaitMsg", []interface{}{callMsg, 0})
		if err != nil {
			return nil, err
		}
	}
	return map[string]interface{}{
		"Height":    result.Get("Height").Value(),
		"Receipt":   result.Get("Receipt").Value(),
		"ReturnDec": result.Get("ReturnDec").Value(),
	}, nil
}
